package com.sun.app.sun.activity;
...
public class ChooseAreaActivity extends Activity {
    ...
    private boolean isFromWeatherActivity;//是否是从WeatherActivity中跳转过来的
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        requestWindowFeature(Window.FEATURE_NO_TITLE);//隐藏标题栏，一定在setContentView前面
        setContentView(R.layout.choose_area);
        listView = (ListView)findViewById(R.id.list_view);
        titleText = (TextView)findViewById(R.id.title_text);
        adapter = new ArrayAdapter<String>(this,android.R.layout.simple_list_item_1,dataList);
        listView.setAdapter(adapter);
        sunDB = SunDB.getInstance(this);
    ...
    //查询并显示全国所有的省，优先从数据库查询，如果没有查询到再去服务器上查询
    private void queryProvinces(){
        provinceList = sunDB.loadProvince();
    ...
    //查询并显示选中省内所有的市，优先从数据库中查询，如果没有查询到再到服务器上查询
    private void queryCities(){
        cityList = sunDB.loadCities(selectedProvince.getId());
        if(cityList.size() > 0){
            dataList.clear();
    ...
    //查询并显示选中市内的所有县，优先从数据库查询，查询不到再到服务器上查
    private  void queryCounties(){
        countyList = sunDB.loadCounties(selectedCity.getId());
